...
И сразу честная рамка, чтобы никто не принял это за больше, чем оно есть. End-to-end измерен пока только GF16; остальные ступени — это спецификация и, где-то, RTL, без бенчмарков. Красивый сплит из 1/φ не означает оптимальности — известна работа Daubechies et al. (Golden Ratio Encoder), где φ не единственное «хорошее» основание; реальный плюс скромнее — «безмультипликаторность» (из φ² = φ + 1 множитель усиления схлопывается в единицу). Сопутствующий каталог-оракул Corona — RTL-дизайн (RTL + GDS + формальные доказательства + cocotb), проверенный в симуляции и формально, дизайн-артефакт, а не изготовленный кристалл; аппаратная верификация — на FPGA AX7203. Всё это я держу под явными статус-метками (`Verified` / `Empirical fit` / `Open conjecture`).
...
Число записывается так же позиционно, как и обычно, только вес i-й позиции равен 3ⁱ, а цифры берутся из {−1, 0, +1}. Например (значения из статьи Брусенцова): десятичное 13 — это `+ + +`, 7 — это `+ − +`, 6 — это `+ − 0`, а −6 — это `− + 0` ([Брусенцов, SoRuCom 2006](https://dl.ifip.org/db/conf/ifip9/sorucom2006/BrusentsovA06.pdf)).
...
Важная честная оговорка: «экономнее по основанию» не значит «быстрее и дешевле в железе прямо сейчас». Реальный мир выбрал двойку по совокупности инженерных и экономических причин — об этом дальше.
...
И ещё одна честная деталь про «железо». В сопутствующем каталоге-оракуле Corona есть декодер формата `bitnet` — то есть троичные представления учтены на уровне дизайна. Но Corona — это **RTL-дизайн** (RTL + GDS + формальные доказательства + cocotb-тесты), проверенный в симуляции и формально, дизайн-артефакт. Изготовленного кристалла нет и он не планируется. Поэтому никаких «доказано на кремнии» — корректная формулировка: проверено в симуляции и формально, аппаратная проверка — на FPGA.
...
- В моём φ-семействе троичность присутствует как GFTernary (`{−φ, 0, +φ}`, спека + Rust/Zig) и TF3 (контейнер, только спека); Corona — RTL-дизайн в симуляции, не изготовленный кристалл. Никаких «доказано на кремнии».
...
- помочь с интеграцией форматов в ваши кернелы и инструментарий (есть Rust-референс, Verilog/FPGA-реализация (плата AX7203) и открытый каталог совместимости с бит-точными конформанс-векторами и Corona-дизайн как оракул);
