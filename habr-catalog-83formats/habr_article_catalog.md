...
Второе применение — **кодоген под новое железо**. Из одной спеки выводятся декодеры на нужном языке, включая RTL. Цепочка `SSOT → кодоген → RTL → FPGA` целиком механическая; CI-гейт `rom_readback` ловит любое расхождение между спекой и тем, что реально лежит в ПЗУ декодера.
...
- **Corona — это законченный RTL-дизайн-оракул (RTL + GDS + формальная верификация + cocotb), дизайн-артефакт, а не изготовленный кристалл.** Проверено в симуляции и формально; изготовление кремния не планируется; аппаратная верификация — на FPGA AX7203.
