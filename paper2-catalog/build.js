...
children.push(H2('2.3. Смежные, но различные направления: вендор-специфичные модели и tolerance-верификация'));
children.push(body([
  t('Два недавних направления легко спутать с настоящей работой, поэтому их важно явно разграничить. '),
  b('Формальные модели тензорных ядер '),
  t('(например, arXiv:2512.07004) формализуют числовое поведение конкретных вендор-блоков GEMM (порядок аккумуляции, промежуточное округление, разбиение на тайлы) для воспроизводимости аппаратного исполнения. Настоящий каталог, напротив, вендор-нейтрален и фиксирует битоточные векторы НА УРОВНЕ ПРЕДСТАВЛЕНИЯ отдельных форматов (кодирование/декодирование), а не конвейер GEMM конкретного вендора; это дополняющие, а не конкурирующие слои. '),
  b('Tolerance-based верификация (TAO, '),
  t('arXiv:2510.16028) проверяет числовые ядра с допуском (approximate equivalence в пределах заданной ошибки), тогда как пакеты соответствия настоящего каталога требуют '),
  b('битоточного '),
  t('совпадения (abs_error = 0) с независимым оракулом. Таким образом, TAO и настоящая работа решают разные задачи: допустимость приближённого равенства против детерминированного битоточного тождества.')
]));

children.push(H2('2.4. Разрыв'));
children.push(body([
