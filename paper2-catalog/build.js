...
children.push(body([ b('Вклад 2: шесть битоточных пакетов соответствия. '),
  ...
  mono('tt-lang-t27'), t(' v0.3.1; остальные четыре представлены в текущем предварительном выпуске, доступном по адресу '),
...
children.push(body([ b('Структура статьи. '),
  t('Раздел 2 рассматривает соответствующий ландшафт стандартов. Раздел 3 описывает устройство каталога. Раздел 4 определяет методологию пакетов соответствия. Раздел 5 поочерёдно представляет каждый из шести пакетов. Раздел 6 предоставляет кросс-уолк к IEEE P3109. Раздел 7 обсуждает интерпретационный разрыв как проектную особенность. Раздел 8 охватывает воспроизводимость и происхождение. Раздел 9 намечает будущую работу. Раздел 10 формулирует ограничения.') ]));

children.push(body([ b('Историческая преемственность. '),
  t('Один из шести пакетов соответствия — GoldenFloat 16 (GF16) — относится к отечественной линии исследований оригинальных систем представления чисел, начатой троичной ЭВМ «Сетунь» под руководством Н. П. Брусенцова (Вычислительный центр МГУ, 1958) — первой в мире машиной на симметричном троичном коде (−1, 0, +1), а в «Сетунь-70» (1970) — с введённым троичным форматом «трайт» и оптимизированным диапазоном мантиссы нормализованного числа. По доступным открытым источникам, GF16 — один из первых именованных числовых форматов российского авторства, доведённых до уровня публикации и аппаратной реализации (FPGA).') ]));
...
children.push(body([
  t('где φ = (1 + √5)/2 — золотое сечение. Это тождество представлено и контекстуализировано в препринте GoldenFloat как численно обоснованный L₂-якорь. Значение 3,0 точно представимо во всех шести форматах пакетов (оно попадает в нормальный диапазон с нулевой ошибкой мантиссы для всех шести раскладок), что делает его надёжной однострочной проверкой корректности между пакетами.'),
]));
children.push(body([
...
// =================== 5. Six Packs ===================
children.push(H1('5. Шесть пакетов соответствия'));
children.push(body([ t('Таблица 4 даёт сводку по всем шести пакетам.') ]));
...
children.push(caption('Таблица 4. Шесть пакетов с первого взгляда (T4).'));
...
children.push(bullet([ b('gHashTag/t27 '), t('('), mono('github.com/gHashTag/t27'), t('): единый источник истины (SSOT) для каталога и пакетов соответствия уровня Tier-1. Здесь располагаются все JSON-файлы каталога, файлы пакетов, проверки инвариантов и шаблоны кодогенерации.') ]));
...
children.push(body([
  ...
  t(' фиксирует все шесть значений SHA-256 пакетов, якорь версии ml_dtypes и ссылку на согласование с P3109 в едином машиночитаемом документе. Последующие потребители могут проверить целостность пакета, заново вычислив SHA-256 канонического JSON-файла и сравнив с записью в манифесте.'),
]));
...
children.push(bullet([ t('Типы Posit/Unum III через оракул libtakum.') ]));
children.push(bullet([ t('Варианты GoldenFloat от GF4 до GF256.') ]));
